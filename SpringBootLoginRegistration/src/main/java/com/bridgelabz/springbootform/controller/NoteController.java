...
public class NoteController {
	...
	public Note noteCreate(@RequestBody Note note, HttpServletRequest request,@PathVariable String token) {
	...
	public Note noteUpdate(@RequestBody Note note,HttpServletRequest request,@PathVariable String token) {
		//String token=request.getHeader("token");
		return noteService.updateNote(note, token);

	}

	@RequestMapping(value = "/notedelete/{noteId}/{token}", method = RequestMethod.DELETE)
	...
	@RequestMapping(value="/notelist/{token}",method=RequestMethod.GET)
